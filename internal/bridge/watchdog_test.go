...
func TestDeadEntries(t *testing.T) {
	st := &state.State{Bridges: map[string]*state.Entry{
		...
		"alive": {Handle: "alive", PID: fakeBridgePID(t, "alive")}, // alive（argv が bridge に見えるダミー）
...
// TestRunWatchdogDaemonRunsAndStops はデーモンループが少なくとも 1 回実行され、
// コンテキストキャンセルで正常終了することを確認する。
func TestRunWatchdogDaemonRunsAndStops(t *testing.T) {
	t.Setenv("AGENT_HUB_HOME", t.TempDir())
	...
	// 短い interval でデーモンを実行し、タイムアウト後に終了することを確認する
	out := captureStdout(t, func() {
		if err := runWatchdogDaemonCtx(ctx, 50*time.Millisecond, ""); err != nil {
			t.Errorf("runWatchdogDaemonCtx: %v", err)
		}
	})

	if !strings.Contains(out, "watchdog daemon: starting") {
		t.Errorf("expected daemon start message, got: %q", out)
	}
	if !strings.Contains(out, "watchdog daemon: shutting down") {
		t.Errorf("expected daemon shutdown message, got: %q", out)
	}
	// 少なくとも 1 回は alive チェックが実行されていること
	if !strings.Contains(out, "bridges alive") {
		t.Errorf("expected at least one alive check, got: %q", out)
	}
}

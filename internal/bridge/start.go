...
func runStartOne(handle string, timeout int) error {
	...
	return runSpawn(handle, bridgeType, cfg.Workdir, cfg.Tenant, cfg.DisplayName, modelSpec{ID: cfg.Model, Source: modelSourceConfig}, timeout)
}
...
func runStartAll(timeout int) error {
	...
	for _, cfg := range cfgs {
		...
		if err := runSpawn(cfg.Handle, bridgeType, cfg.Workdir, cfg.Tenant, cfg.DisplayName, modelSpec{ID: cfg.Model, Source: modelSourceConfig}, timeout); err != nil {
